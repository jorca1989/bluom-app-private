...
export const forceReset = mutation({
    ...
    handler: async (ctx) => {
        ...
        for (const ex of exercises) {
            const typeStr = ex.type as string; // Avoid union narrowing issues
            await ctx.db.insert("exerciseLibrary", {
                ...ex,
                nameLower: ex.name.toLowerCase(),
                type: ex.type as "strength" | "cardio" | "hiit" | "yoga",
                muscleGroups: [...ex.muscleGroups], // Convert readonly to mutable
        ...
        return `RESET SUCCESSFUL! Full library of ${exercises.length} exercises seeded.`;
    },
});
