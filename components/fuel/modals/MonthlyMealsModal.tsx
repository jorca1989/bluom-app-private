...
function build30DayPlan(dietKey: DietKey, dict: Record<string, any>): DayTemplate[] {
  const weekPatterns = dict[dietKey] ?? dict['balanced'] ?? GENERIC_WEEK['balanced'];
...
export default function MealHubScreen() {
  ...
  const planData: DayTemplate[] = useMemo(() => {
    ...
    let dict = t('monthlyMeals', { returnObjects: true }) as any;
    if (!dict || typeof dict !== 'object' || !dict['high_protein']) {
      dict = GENERIC_WEEK;
    }
    return build30DayPlan(dietKey, dict);
  }, [isPro, activePlans, dietKey, t]);
