  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={{ marginRight: 12 }}>
      ...
      >
        ...
        <Text style={wcS.weekLabel}>WEEK {theme.week}</Text>
        <Text style={wcS.title}>{theme.theme}</Text>
        <Text style={wcS.tagline}>{theme.tagline}</Text>
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.82}>
      <View style={[drS.row, isToday && drS.todayRow, isCompleted && drS.completedRow]}>
        ...
        <View style={drS.info}>
          <Text style={drS.title} numberOfLines={1}>{plan.title}</Text>
          <Text style={drS.meta}>{plan.focus} · {totalMin > 0 ? `${totalMin} min` : 'Evening'}</Text>
        </View>
    ...
    <View style={acS.card}>
      <View style={acS.header}>
        <View style={[acS.pillarBadge, { backgroundColor: pillar.bg }]}>
          ...
          <Text style={[acS.pillarText, { color: pillar.color }]}>{pillar.label}</Text>
        </View>
        <View style={acS.metaRow}>
          ...
          <Text style={[acS.intText, { color: intensity.color }]}>{intensity.label}</Text>
        </View>
      </View>
      <Text style={acS.title}>{activity.title}</Text>
      <Text style={acS.desc}>{activity.description}</Text>
      <View style={acS.whyBox}>
        <Ionicons name="flask-outline" size={12} color="#7c3aed" />
        <Text style={acS.why}>{activity.why}</Text>
      </View>
    </View>
  );
}
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }} edges={['top']}>
        ...
        >
          ...
          <Text style={ddS.dayLabel}>DAY {plan.day}</Text>
          <Text style={ddS.title}>{plan.title}</Text>
          <Text style={ddS.focus}>{plan.focus}</Text>
        </LinearGradient>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
          ...
          <View style={ddS.affirmBox}>
            ...
            <Text style={ddS.affirmText}>"{plan.affirmation}"</Text>
          </View>

          {/* Activities */}
          <Text style={ddS.sectionTitle}>Today's Practices</Text>
          {plan.activities.map(a => <ActivityCard key={a.id} activity={a} />)}

          {/* Reflection */}
          <View style={ddS.reflectionBox}>
            <Text style={ddS.reflectionLabel}>📝  Evening Reflection</Text>
            <Text style={ddS.reflectionPrompt}>{plan.reflectionPrompt}</Text>
          </View>
          ...
          <View style={ddS.legendRow}>
            {Object.entries(PILLAR_META).map(([key, meta]) => (
              <View key={key} style={[ddS.legendItem, { backgroundColor: meta.bg }]}>
                ...
                <Text style={[ddS.legendText, { color: meta.color }]}>{meta.label}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
      ...
      >
        ...
        >
          ...
          <View style={ps.progressWrap}>
            ...
            <Text style={ps.progressText}>Day {currentDay - 1} of 28 complete · {progressPct}%</Text>
          </View>
        </LinearGradient>
            ...
            >
              ...
              >
                <View style={ps.todayLeft}>
                  <Text style={ps.todayLabel}>TODAY · DAY {currentDay}</Text>
                  <Text style={ps.todayTitle}>{ALL_DAYS[currentDay - 1]?.title ?? ''}</Text>
                  <Text style={ps.todayFocus}>{ALL_DAYS[currentDay - 1]?.focus ?? ''}</Text>
                </View>
                <View style={ps.todayRight}>
                  <View style={ps.startBtn}>
                    <Ionicons name="play" size={18} color="#8b5cf6" />
                    <Text style={ps.startBtnText}>Start</Text>
                  </View>
                </View>
              </LinearGradient>
            </TouchableOpacity>
        ...
        <View style={[ps.section, { marginTop: 28 }]}>
          <Text style={ps.sectionTitle}>Pillar Balance</Text>
          <Text style={ps.sectionSub}>Activities completed across each dimension</Text>
            ...
            {(Object.entries(PILLAR_META) as [PillarKey, typeof PILLAR_META[PillarKey]][]).map(([key, meta]) => {
              ...
              return (
                <View key={key} style={[ps.pillarCard, { backgroundColor: meta.bg }]}>
                  ...
                  <Text style={[ps.pillarLabel, { color: meta.color }]}>{meta.label}</Text>
        ...
        <View style={{ marginTop: 28 }}>
          <View style={[ps.section, { marginBottom: 14 }]}>
            <Text style={ps.sectionTitle}>4-Week Overview</Text>
          </View>
          <ScrollView
        ...
        <View style={[ps.section, { marginTop: 24 }]}>
          <View style={ps.weekHeaderRow}>
            <View>
              <Text style={ps.sectionTitle}>{activeWeek.theme}</Text>
              <Text style={ps.sectionSub}>{activeWeek.tagline}</Text>
            </View>
            <LinearGradient colors={activeWeek.colors} style={ps.weekBadge}>
              <Text style={ps.weekBadgeText}>Week {activeWeek.week}</Text>
            </LinearGradient>
          </View>
        ...
        <View style={ps.section}>
          <View style={ps.researchBox}>
            ...
            <Text style={ps.researchText}>
              This programme draws on evidence-based practices from CBT, ACT, positive psychology, neuroscience, and somatic therapy. It is not a substitute for professional mental health care.
            </Text>
          </View>
        </View>
