...
function getKetosisState(netCarbs: number, t: any): { state: string; color: string; bgColor: string } {
  ...
  if (netCarbs < 50) return { state: t('metabolic.nutritionalKetosis', 'Nutritional Ketosis'), color: '#2563eb', bgColor: '#eff6ff' }; // Blue
  return { state: t('metabolic.glucoseBurning', 'Glucose Burning'), color: '#475569', bgColor: '#f8fafc' }; // Slate
}
...
export default function MetabolicHub() {
  ...
  async function handleSaveCheckIn() {
    ...
    if (checkInSugarFree === null) {
      Alert.alert(t('metabolic.dailyCheckin', 'Daily check-in'), t('sugarControl.checkinNeededBody', 'Choose Sugar‑free or Had sugar.'));
      return;
    }
    setSaving(true);
    try {
    ...
    <SafeAreaView className="flex-1 bg-[#F5F4F0]" edges={['top', 'bottom']}>
      ...
      >
        ...
        <View style={{ backgroundColor: ketoState.bgColor }} className="mb-4 border border-slate-200 rounded-2xl p-4 flex-row items-center justify-between">
          <View>
            <Text style={{ color: ketoState.color }} className="font-extrabold text-xs uppercase tracking-wider mb-1">{t('metabolic.currentState', 'Current State')}</Text>
        ...
        <View className="bg-white border border-slate-200 rounded-2xl p-4 mb-4">
          <View className="flex-row items-center justify-between">
            ...
            <View className="bg-orange-50 border border-orange-200 rounded-full px-3 py-1 flex-row items-center gap-1">
              ...
              <Text className="text-orange-800 font-extrabold">{streaks?.sugar ?? streak}{t('sugarControl.dStreak', 'd streak')}</Text>
            </View>
          </View>
        ...
        >
          <View className="flex-row items-center gap-3">
            ...
            <View>
              <Text className="text-white font-extrabold text-lg">{t('metabolic.scanFood', 'Scan Food')}</Text>
              <Text className="text-white/70 font-bold text-xs">{t('metabolic.scanFoodDesc', 'Detect Hidden Sugars & Macros')}</Text>
            </View>
          </View>
        ...
        <SafeAreaView className="flex-1 bg-white" edges={['top']}>
          ...
          <ScrollView className="p-5">
            ...
            <TextInput
              ...
              placeholder={t('sugarControl.notesPlaceholder', 'How are you feeling? Triggers?')}
              multiline
              numberOfLines={4}
            ...
            >
              <Text className="text-white font-black text-lg">{saving ? t('sugarControl.saving', 'Saving...') : t('sugarControl.saveCheckin', 'Save Check-in')}</Text>
            </TouchableOpacity>
          </ScrollView>
        </SafeAreaView>
      </Modal>
        ...
        <View className="flex-1 bg-black/50 justify-center p-4">
          <View className="bg-white rounded-3xl p-6 h-[70%]">
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text className="text-slate-500 font-medium mb-4">{t('metabolic.greenListIntro', 'Stick to these foods to stay in Deep/Nutritional Ketosis.')}</Text>

              <View className="mb-4">
                <Text className="text-emerald-600 font-black mb-2 uppercase tracking-wider text-xs">{t('metabolic.healthyFats', 'Healthy Fats')}</Text>
                <Text className="text-slate-800 font-semibold leading-6">{t('metabolic.healthyFatsList', '• Avocado / Avocado Oil\n• Olive Oil / Coconut Oil\n• Butter / Ghee\n• MCT Oil')}</Text>
              </View>

              <View className="mb-4">
                <Text className="text-blue-600 font-black mb-2 uppercase tracking-wider text-xs">{t('metabolic.proteins', 'Proteins')}</Text>
                <Text className="text-slate-800 font-semibold leading-6">{t('metabolic.proteinsList', '• Fatty Fish (Salmon, Mackerel)\n• Eggs (Whole)\n• Beef / Steak (Fatty cuts)\n• Chicken Thighs (Skin on)')}</Text>
              </View>

              <View className="mb-4">
                <Text className="text-purple-600 font-black mb-2 uppercase tracking-wider text-xs">{t('metabolic.lowCarbVeggies', 'Low Carb Veggies')}</Text>
                <Text className="text-slate-800 font-semibold leading-6">{t('metabolic.lowCarbVeggiesList', '• Spinach / Kale\n• Broccoli / Cauliflower\n• Zucchini\n• Asparagus')}</Text>
              </View>

              <View>
                <Text className="text-rose-600 font-black mb-2 uppercase tracking-wider text-xs">{t('metabolic.avoid', 'Avoid / Limit')}</Text>
                <Text className="text-slate-800 font-semibold leading-6">{t('metabolic.avoidList', '• Bread, Pasta, Rice\n• Potatoes\n• Sugary Fruits (Bananas, Grapes)\n• Processed Snacks')}</Text>
              </View>
            </ScrollView>
          </View>
        </View>
      ...
      <ProUpgradeModal
        ...
        title={t('common.unlockWithPro', 'Unlock with Pro')}
        message={t('metabolic.proMessage', 'Upgrade to Pro to unlock premium Metabolic insights.')}
