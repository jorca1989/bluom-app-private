    ...
    <SafeAreaView style={s.screen} edges={['top']}>
        ...
        <View style={{ flex: 1, backgroundColor: themeColors.bg }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
            ...
            <View style={s.carbLoadToggle}>
              ...
              <Switch value={carbLoad} onValueChange={setCarbLoad} trackColor={{ false: '#e2e8f0', true: '#d97706' }} thumbColor="#fff" />
            </View>
          </ScrollView>
        </View>
      </Modal>
      ...
      <LinearGradient colors={[themeColors.surface, themeColors.bg]} style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[s.headerSub, { color: mc.accent, fontWeight: '800' }]}>{mc.emoji} {mc.label} · {mc.tagline}</Text>
        </View>
          ...
          <>
            ...
            <LinearGradient colors={mc.gradient} style={s.modeHero}>
              ...
              {profile?.trainingMode === 'enhanced' && (
                <View style={s.cyclePhaseRow}>
                  {phases.map((p: any) => (
                    <TouchableOpacity key={p.id} style={[s.cPhaseChip, cyclePhase === p.id && { backgroundColor: 'rgba(255,255,255,0.25)', borderColor: '#fff' }]} onPress={() => setCyclePhase(p.id)}>
                      <Text style={[s.cPhaseLabel, { color: cyclePhase === p.id ? '#ffffff' : 'rgba(255,255,255,0.6)' }]}>{p.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
            ...
            {[
              ...
              },
              {
                ...
                sub: t('mensHealth.pelvicActionSub', 'Core, função erétil & urinária'), color: mc.accent, onPress: () => setShowPelvic(true),
              },
              {
                ...
                color: mc.accent, onPress: () => setShowSupps(true),
              },
              ...(profile?.trainingMode === 'enhanced' ? [{
                icon: 'heart-circle-outline', label: t('mensHealth.healthMarkers', 'Health Markers'),
                sub: 'BP · HCT · ALT · LDL · PSA', color: mc.accent, onPress: () => setShowMarkers(true),
              }] : []),
              ...(profile?.trainingMode === 'athlete' && profile.competitionFocus === 'bodybuilding' ? [{
                ...
                sub: t('mensHealth.peakWeekSub', 'Carb load · water · stage prep'), color: mc.accent, onPress: () => setShowPeakWeek(true),
              }] : []),
            ].map((a, i) => (
...
const createS = (c: ThemeColors) => StyleSheet.create({
  ...
  // Quiz
  quizScreen: { flex: 1, backgroundColor: c.bg },
  ...
  quizProgress: { flex: 1, height: 4, backgroundColor: c.border, borderRadius: 2, overflow: 'hidden' },
  quizProgressFill: { height: '100%', backgroundColor: '#60a5fa', borderRadius: 2 },
  quizStepNum: { fontSize: 12, fontWeight: '700', color: c.text, width: 32, textAlign: 'right' },
  ...
  quizQuestion: { fontSize: 22, fontWeight: '800', color: c.text, marginBottom: 8, lineHeight: 30 },
  quizHint: { fontSize: 12, color: c.textMuted, marginBottom: 16, fontStyle: 'italic' },
  quizOptions: { gap: 10 },
  quizOption: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: c.surface, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: c.border },
  quizOptionIcon: { fontSize: 24 },
  quizOptionLabel: { fontSize: 15, fontWeight: '700', color: c.text },
  quizOptionDesc: { fontSize: 11, color: c.textMuted, marginTop: 2 },

  // Header
  header: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 8, borderBottomWidth: 1, borderBottomColor: c.border },
  ...
  scoreChipLbl: { fontSize: 8, color: c.textMuted, fontWeight: '700' },
  retakeBtn: { width: 34, height: 34, borderRadius: 10, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
  ...
  modeHeroScoreLbl: { fontSize: 9, color: 'rgba(255,255,255,0.7)', fontWeight: '700', marginTop: 2 },
  tOptBar: { height: 5, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 3, overflow: 'hidden' },
  ...
  cPhaseChip: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.3)', backgroundColor: 'rgba(255,255,255,0.1)' },
  cPhaseLabel: { fontSize: 11, fontWeight: '700', color: '#fff' },
  cPhaseChipLarge: { flex: 1, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: c.border, backgroundColor: c.surface, alignItems: 'center' },
  ...
  compBtnTxt: { fontSize: 12, color: c.text, fontWeight: '800' },

  // Cycle phase card
  cyclePhaseCard: { backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 14, borderLeftWidth: 4 },
  cyclePhaseTitle: { fontSize: 14, fontWeight: '800', marginBottom: 4 },
  cyclePhaseDesc: { fontSize: 13, color: c.textMuted, lineHeight: 19 },

  // Actions
  actionTile: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: c.surface, borderRadius: 16, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: c.border },
  ...
  modalSub: { fontSize: 13, color: c.textMuted, marginTop: 4 },
  modalClose: { width: 34, height: 34, borderRadius: 10, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
  tOptLive: { alignItems: 'center', backgroundColor: c.surface, borderRadius: 16, padding: 16, marginVertical: 16, borderWidth: 1, borderColor: c.border },
  tOptLiveLabel: { fontSize: 10, fontWeight: '800', color: c.textMuted, letterSpacing: 1, marginBottom: 4 },
  ...
  sliderLabel: { fontSize: 13, fontWeight: '700', color: c.textMuted },
  sliderVal: { fontSize: 16, fontWeight: '900' },
  sliderTrack: { height: 4, backgroundColor: c.border, borderRadius: 2, overflow: 'hidden', marginBottom: 10 },
  sliderFill: { height: '100%', borderRadius: 2 },
  sliderDots: { flexDirection: 'row', gap: 3 },
  sliderDot: { flex: 1, height: 28, borderRadius: 6, borderWidth: 1, borderColor: c.border, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
  sliderDotTxt: { fontSize: 10, fontWeight: '700', color: c.text },
  moodSync: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: c.surface, borderRadius: 10, padding: 10, marginBottom: 16 },
  ...
  suppGroupLabel: { fontSize: 13, fontWeight: '800', color: c.textMuted },
  suppCard: { backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: c.border },
  suppTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 },
  suppName: { fontSize: 15, fontWeight: '800', color: c.text },
  evidenceBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },
  evidenceTxt: { fontSize: 10, fontWeight: '800' },
  suppDose: { fontSize: 12, color: c.textMuted, marginBottom: 6, fontWeight: '600' },
  suppReason: { fontSize: 13, color: c.textMuted, lineHeight: 18 },
  suppDisclaimer: { flexDirection: 'row', gap: 8, backgroundColor: c.surface, borderRadius: 12, padding: 12, marginTop: 8 },
  suppDisclaimerTxt: { flex: 1, fontSize: 11, color: c.textMuted, lineHeight: 16 },

  // Health markers modal
  markerCard: { backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: c.border },
  ...
  peakDayBtn: { flex: 1, minWidth: 44, paddingVertical: 10, borderRadius: 10, borderWidth: 1, borderColor: c.border, backgroundColor: c.surface, alignItems: 'center' },
  peakDayTxt: { fontSize: 11, fontWeight: '700', color: c.textMuted },
  peakDayCard: { backgroundColor: c.surface, borderRadius: 14, padding: 16, borderWidth: 1, borderColor: c.border },
  peakDayCardTitle: { fontSize: 15, fontWeight: '800', color: '#fcd34d', marginBottom: 8 },
  peakDayCardBody: { fontSize: 13, color: c.textMuted, lineHeight: 20 },
  carbLoadToggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: c.surface, borderRadius: 12, padding: 14, marginTop: 14 },
  carbLoadLabel: { fontSize: 14, fontWeight: '700', color: c.text },

  // Track tab
  historyChart: { backgroundColor: c.surface, borderRadius: 18, padding: 16, marginBottom: 14 },
  historyRow: { flexDirection: 'row', alignItems: 'flex-end', marginBottom: 14, gap: 8 },
  historyDate: { fontSize: 10, fontWeight: '700', color: c.textMuted, width: 36 },
  ...
  historyBarLabel: { fontSize: 9, fontWeight: '700', color: c.text, marginTop: 3 },
  historyScore: { fontSize: 11, fontWeight: '800', color: c.textMuted, width: 34, textAlign: 'right' },

  pillarCard: { backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 10 },
  pillarTop: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
  pillarEmoji: { fontSize: 18 },
  pillarName: { flex: 1, fontSize: 14, fontWeight: '700', color: c.text },
  pillarScore: { fontSize: 16, fontWeight: '900' },
  pillarBar: { height: 4, backgroundColor: c.border, borderRadius: 2, overflow: 'hidden', marginBottom: 8 },
  pillarBarFill: { height: '100%', borderRadius: 2 },
  pillarTip: { fontSize: 12, color: c.textMuted, lineHeight: 18 },

  cycleTracker: { backgroundColor: c.surface, borderRadius: 18, padding: 16, marginBottom: 14 },
  cycleTrackerLabel: { fontSize: 12, fontWeight: '800', color: c.textMuted, textTransform: 'uppercase', marginBottom: 10 },
  cycleWeekRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, marginBottom: 10 },
  cycleWeekLabel: { fontSize: 14, fontWeight: '700', color: c.text },
  cycleWeekBtns: { flexDirection: 'row', gap: 8 },
  cycleWeekBtn: { width: 34, height: 34, borderRadius: 10, backgroundColor: c.border, alignItems: 'center', justifyContent: 'center' },
  cycleWeekBtnTxt: { fontSize: 18, fontWeight: '900', color: c.text },
  cycleProgressBar: { height: 5, backgroundColor: c.border, borderRadius: 3, overflow: 'hidden' },
  cycleProgressFill: { height: '100%', backgroundColor: '#60a5fa', borderRadius: 3 },

  markerChip: { flexDirection: 'row', alignItems: 'center', backgroundColor: c.surface, borderRadius: 12, padding: 12, marginBottom: 8, gap: 10 },
  markerChipIcon: { fontSize: 18 },
  markerChipName: { fontSize: 13, fontWeight: '700', color: c.text },
  markerChipNormal: { fontSize: 11, color: c.textMuted },
  markerChipLogged: { fontSize: 13, fontWeight: '800', color: '#4ade80' },
  markerChipEmpty: { fontSize: 13, fontWeight: '700', color: c.text },
  ...
  peakWeekCard: { backgroundColor: c.surface, borderRadius: 16, padding: 18, borderWidth: 1, borderColor: '#92400e' },
  peakWeekTitle: { fontSize: 16, fontWeight: '800', color: '#fcd34d', marginBottom: 6 },
  peakWeekSub: { fontSize: 13, color: c.textMuted, lineHeight: 19, marginBottom: 10 },
  peakWeekCta: { fontSize: 13, fontWeight: '800', color: '#fcd34d' },

  // Insights
  insightCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 10, borderLeftWidth: 4 },
  insightEmoji: { fontSize: 22, marginTop: 2 },
  insightTitle: { fontSize: 14, fontWeight: '800', color: c.text, marginBottom: 4 },
  insightBody: { fontSize: 13, color: c.textMuted, lineHeight: 19 },

  proBlurWrap: { borderRadius: 16, overflow: 'hidden', height: 160, backgroundColor: c.surface, marginBottom: 14 },
  ...
  proBlurBtnTxt: { color: c.text, fontWeight: '800', fontSize: 14 },

  // Learn
  articleCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: c.surface, borderRadius: 14, padding: 14, marginBottom: 10, gap: 12, borderWidth: 1, borderColor: c.border },
  ...
  articleHeader: { flexDirection: 'row', gap: 12, padding: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: c.border },
  ...
  articleClose: { width: 34, height: 34, borderRadius: 10, backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
  articleBody: { fontSize: 15, color: c.textMuted, lineHeight: 24, marginBottom: 20 },
  articleDisclaimer: { flexDirection: 'row', gap: 8, backgroundColor: c.surface, borderRadius: 12, padding: 12 },
  articleDisclaimerTxt: { flex: 1, fontSize: 11, color: c.textMuted, lineHeight: 16 },

  emptyTxt: { fontSize: 13, color: c.text, textAlign: 'center', paddingVertical: 30, fontStyle: 'italic' },
});
