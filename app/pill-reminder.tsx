...
export default function PillReminderScreen() {
  ...
  const handleAddSchedule = async () => {
    ...
    const savedPillName = newName.trim();
    setNewName(''); setNewTime('08:00'); setNewDays([0,1,2,3,4,5,6]);
    Alert.alert(t('pillReminder.added'), t('pillReminder.savedMsg', { name: savedPillName }));
  };

  const MONTH_NAMES = [
    t('pillReminder.months.jan'), t('pillReminder.months.feb'), t('pillReminder.months.mar'),
    t('pillReminder.months.apr'), t('pillReminder.months.may'), t('pillReminder.months.jun'),
    t('pillReminder.months.jul'), t('pillReminder.months.aug'), t('pillReminder.months.sep'),
    t('pillReminder.months.oct'), t('pillReminder.months.nov'), t('pillReminder.months.dec')
  ];
  const DAY_NAMES = [
    t('pillReminder.daysInitials.sun'), t('pillReminder.daysInitials.mon'), t('pillReminder.daysInitials.tue'),
    t('pillReminder.daysInitials.wed'), t('pillReminder.daysInitials.thu'), t('pillReminder.daysInitials.fri'),
    t('pillReminder.daysInitials.sat')
  ];
  ...
  return (
    <SafeAreaView style={[s.screen, { backgroundColor: themeColors.bg }]} edges={['top']}>
                  ...
                  <View key={i} style={[s.pillCard, { borderLeftColor: pill.color }]}>
                    ...
                    <View style={[s.statusBadge, { backgroundColor: log?.taken ? '#dcfce7' : '#fee2e2' }]}>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: log?.taken ? '#15803d' : '#b91c1c' }}>
                        {log?.taken ? `✓ ${t('pillReminder.status.taken')}` : `✗ ${t('pillReminder.status.missed')}`}
                      </Text>
                    </View>
                  </View>
                );
      ...
      <View style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.headerTitle}>{t('pillReminder.title')}</Text>
          <Text style={s.headerSub}>{t('pillReminder.tracker')}</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={[s.scroll, { paddingBottom: insets.bottom + 40 }]}
          ...
          <>
            ...
            <View style={s.statsRow}>
              <View style={s.statCard}>
                ...
                <Text style={s.statLbl}>{t('pillReminder.dayStreak')}</Text>
              </View>
              <View style={s.statCard}>
                <Text style={s.statVal}>{adherenceThisMonth}%</Text>
                <Text style={s.statLbl}>{t('pillReminder.thisMonth')}</Text>
              </View>
              <View style={s.statCard}>
                <Text style={s.statVal}>{(todayPills ?? []).filter(p => p.taken).length}/{(todayPills ?? []).length}</Text>
                <Text style={s.statLbl}>{t('pillReminder.today')}</Text>
              </View>
            </View>

            <Text style={s.sectionTitle}>{t('pillReminder.todaysPills')}</Text>
            {(todayPills ?? []).length === 0 && (
              <Text style={s.emptyTxt}>{t('pillReminder.emptyToday')}</Text>
            )}
        ...
        {activeTab === 'calendar' && (
          <>
            ...
            <View style={s.calLegend}>
              <View style={s.legendItem}><View style={[s.legendDot, { backgroundColor: '#22c55e' }]} /><Text style={s.legendTxt}>{t('pillReminder.status.allTaken')}</Text></View>
              <View style={s.legendItem}><View style={[s.legendDot, { backgroundColor: '#ef4444' }]} /><Text style={s.legendTxt}>{t('pillReminder.status.missed')}</Text></View>
              <View style={s.legendItem}><View style={[s.legendDot, { backgroundColor: '#94a3b8' }]} /><Text style={s.legendTxt}>{t('pillReminder.status.future')}</Text></View>
            </View>
          </>
        )}
        ...
        {activeTab === 'history' && (
          <>
            <Text style={s.sectionTitle}>{t('pillReminder.adherence')}</Text>
            {/* Heatmap grid */}
            ...
            <Text style={s.sectionTitle}>{t('pillReminder.stats')}</Text>
                ...
                <View key={i} style={[s.pillHistCard, { borderLeftColor: pill.color }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.pillHistStats}>
                      ✅ {taken} {t('pillReminder.taken')}  ❌ {missed} {t('pillReminder.missed')}  📅 {pill.reminderTime}
                    </Text>
                  </View>
                </View>
              );
        ...
        {activeTab === 'settings' && (
          <>
            <Text style={s.sectionTitle}>{t('pillReminder.addNewPill')}</Text>
            <View style={s.formCard}>
              <TextInput
                style={s.input}
                placeholder={t('pillReminder.pillNamePlaceholder')}
                placeholderTextColor="#94a3b8"
                value={newName}
                onChangeText={setNewName}
              />
              {/* Color picker */}
              <Text style={s.formLabel}>{t('pillReminder.color')}</Text>
              ...
              {/* Shape selector */}
              <Text style={s.formLabel}>{t('pillReminder.shape')}</Text>
              <View style={s.shapeRow}>
                {(['round','oval','square'] as const).map(sh => (
                  <TouchableOpacity key={sh} style={[s.shapeChip, newShape === sh && s.shapeChipActive]}
                    ...
                    <Text style={[s.shapeChipTxt, newShape === sh && { color: '#fff' }]}>{t(`pillReminder.shapes.${sh}`)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              {/* Time */}
              <Text style={s.formLabel}>{t('pillReminder.reminderTime')}</Text>
              <TextInput
                style={s.input}
                value={newTime}
              ...
              />
              {/* Days of week */}
              <Text style={s.formLabel}>{t('pillReminder.days')}</Text>
              ...
              <TouchableOpacity style={s.addBtn} onPress={handleAddSchedule}>
                <Text style={s.addBtnTxt}>{t('pillReminder.addPillSchedule')}</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={s.notifRow}>
              ...
              <Text style={s.notifLabel}>{t('pillReminder.dailyReminders')}</Text>
            ...
            {/* Existing schedules */}
            <Text style={s.sectionTitle}>{t('pillReminder.yourSchedules')}</Text>
            {(schedules ?? []).map((pill, i) => (
              <View key={i} style={[s.pillCard, { borderLeftColor: pill.color }]}>
                <View style={[s.pillDot, { backgroundColor: pill.color }]} />
                <View style={{ flex: 1 }}>
                  <Text style={s.pillName}>{pill.pillName}</Text>
                  <Text style={s.pillTime}>{pill.reminderTime} · {pill.daysOfWeek.length === 7 ? t('pillReminder.everyDay') : t('pillReminder.daysPerWeek', { count: pill.daysOfWeek.length })}</Text>
                </View>
